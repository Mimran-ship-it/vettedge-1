...
export default function DomainDetailsPage() {
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: title,
          text: `I found this premium domain for sale: ${
            domain.name
          } at $${domain.price.toLocaleString()}`,
          url: url,
        });
        toast({
  ...
  const discountPercentage =
    domain.Actualprice > domain.price
      ? Math.round(
          ((domain.Actualprice - domain.price) / domain.Actualprice) * 100
        )
      : 0;
  ...
  return (
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
            <div>
              <div className="flex items-center gap-3 mb-2">
                <h1 className="text-1xl md:text-4xl font-bold text-gray-900 dark:text-white">
                  {domain.name}
                </h1>
                {domain.isHot && (
    ...
    "
                  ...
                  <TabsTrigger
                    value="metrics"
                    className="
      bg-[#30C97C]/20 text-black dark:text-white
      data-[state=active]:bg-[#30C97C]
      data-[state=active]:text-white
      rounded-lg py-2
      transition
    ...
    "
                  ...
                  <Card className="dark:bg-gray-700   dark:border-gray-600 -mx-4 sm:-mx-8">
                    <CardHeader>
                      <CardTitle className="flex items-center mt-2  gap-2 dark:text-white">
                        <Info className="h-5 w-5" /> Domain Information
                      </CardTitle>
                    </CardHeader>
                    <CardContent>
                ...
                <TabsContent value="metrics" className="mt-4">
                  <Card className=" dark:bg-gray-700 dark:border-gray-600 -mx-4 sm:-mx-8">
                    <CardHeader>
                      <CardTitle className="flex items-center  mt-2 gap-2 dark:text-white">
                        <BarChart3 className="h-5 w-5" /> SEO Metrics
                      </CardTitle>
                    </CardHeader>
                    <CardContent>
                  ...
                  <Card className=" dark:bg-gray-800 dark:border-gray-600 -mx-4 sm:-mx-8">
                    <CardHeader>
                      <CardTitle className="flex items-center gap-2  mt-2 dark:text-white">
                        <Clock className="h-5 w-5" /> Domain History
                      </CardTitle>
                    </CardHeader>
                    <CardContent>
                      <div className="space-y-4">
                        <div className="p-2 sm:p-4  bg-gray-50 dark:bg-gray-600 ">
                          ...
                          <p className="text-sm text-gray-600 dark:text-gray-300">
                            This domain was first registered in{" "}
                            {domain.metrics.year} and has been maintained for{" "}
                            {domain.metrics.age} years.
                          </p>
                        </div>
            ...
            <div className="space-y-6">
              <div className=" dark:bg-gray-700  p-6 shadow-lg border border-blue-100 dark:border-gray-600">
                  ...
                  <div className="flex gap-3">
                    ...
                    >
                      <Heart
                        className={cn(
                          "h-4 w-4 mr-2",
                          domain && isInWishlist(domain._id) && "fill-current"
                        )}
                      />
                      {domain && isInWishlist(domain._id)
                        ? "Remove"
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    <TrustItem
                      icon={<CheckCircle />}
                      text="Secure transfer guaranteed"
                      color="green"
                    />
                    <TrustItem
                      icon={<Clock />}
                      text="Transfer within 24-48 hours"
                      color="blue"
                    />
                    <TrustItem
                      icon={<Shield />}
              ...
              <Card className="dark:bg-gray-700  dark:border-gray-600">
                <CardHeader>
                  <CardTitle className="flex items-center mt-4 gap-2 dark:text-white">
        ...
        <div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <Card className="border-blue-100 bg-blue-50 dark:border-blue-800 dark:bg-blue-900/20 ">
              <CardHeader>
                <CardTitle className="flex items-center gap-2 text-blue-800 dark:text-blue-300">
                  <Eye className="h-5 w-5" /> Traffic Insights
                </CardTitle>
              </CardHeader>
              <CardContent>
                <p className="text-sm text-gray-600 dark:text-gray-300 mb-4">
                  This domain receives approximately{" "}
                  {domain.metrics.monthlyTraffic?.toLocaleString() || "N/A"}{" "}
                  monthly visitors.
                </p>
                <div className="flex items-center justify-between">
          ...
          <Card className="border-purple-100 bg-purple-50 dark:border-purple-800 dark:bg-purple-900/20 ">
            <CardHeader>
              <CardTitle className="flex items-center gap-2 p-2 text-purple-800 dark:text-purple-300">
                <ThumbsUp className="h-5 w-5" /> Overall Score
              </CardTitle>
            </CardHeader>
            <CardContent>
              <p className="text-sm text-gray-600 dark:text-gray-300 mb-4">
              ...
              <div className="w-full bg-gray-200 dark:bg-gray-600  h-2.5">
                <div
                  ...
                  style={{
                    width: `${((domain.metrics.score || 0) / 100) * 100}%`,
                  }}
                ></div>
              </div>
            </CardContent>
          ...
          {domain.type == "aged" && (
            <Card className="border-orange-100 bg-orange-50 dark:border-orange-800 dark:bg-orange-900/20 ">
              <CardHeader>
                <CardTitle className="flex items-center gap-2 p-2 text-orange-800 dark:text-orange-300">
                  <Calendar className="h-5 w-5" /> Age & History
                </CardTitle>
              </CardHeader>
              <CardContent>
                <p className="text-sm text-gray-600 dark:text-gray-300 mb-4">
                  Registered in {domain.metrics.year} with{" "}
                  {domain.metrics.age || "N/A"} years of history.
                </p>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-500 dark:text-gray-400">
                    Clean History
        ...
        {similarDomains.length > 0 && (
          <div className="mt-12">
            <div className="flex justify-between items-center mb-6">
              ...
              <Badge
                variant="outline"
                className="px-3 py-1 dark:border-gray-600"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {similarDomains.map((similarDomain) => (
                <Card
                  key={similarDomain._id}
                  className="overflow-hidden transition-all hover:shadow-lg dark:bg-gray-800 dark:border-gray-700"
                >
                  ...
                  <CardContent className="p-4">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <Badge
                        variant="outline"
                        className="px-2 py-1 text-xs dark:border-gray-600 dark:text-gray-300"
                      >
                        Available
                      </Badge>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-1 mb-3">
                      {similarDomain.tags &&
                        similarDomain.tags.slice(0, 2).map((tag, idx) => (
                          <Badge
                            key={idx}
                            variant="outline"
                    ...
                    <div className="flex justify-between items-center">
                      <div>
                        <span className="text-lg font-bold dark:text-white">
                          ${similarDomain.price.toLocaleString()}
                        </span>
          ...
          domain.tags.length > 0 && (
            <div className="mt-12 bg-white dark:bg-gray-800 xl shadow-sm p-8 text-center">
              <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">
                No Similar Domains Available
              </h3>
              <p className="text-gray-600 dark:text-gray-300 mb-6">
                We couldn't find any available domains similar to {domain.name}{" "}
                at the moment.
              </p>
              <Button asChild className="px-6 py-3">
