...
export default function Domains2Page() {
  ...
  const sortDomains = useCallback(
    (domains: Domain[], sortOption: SortOption): Domain[] => {
    ...
    (filters: ActiveFilters) => {
      ...
      if (filters.tlds.length > 0) {
        result = result.filter((d) =>
          filters.tlds.some((tld) =>
            d.name.toLowerCase().endsWith(tld.toLowerCase())
          )
        );
      }
      ...
      if (filters.scoresRange) {
        result = result.filter((d) => {
          ...
          return (
            d.metrics.score >= filters.scoresRange![0] &&
            d.metrics.score <= filters.scoresRange![1]
          );
        });
      }
      // Trust Flow
      ...
      result = result.filter((d) => {
        ...
        return (
          (d.metrics.authorityLinksCount || 0) >=
          (filters.authorityLinksMin || 0)
        );
      });
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-gray-900">
      <Header />
      <main className="px-4 sm:px-6 lg:px-8 pt-24 pb-28 lg:pb-8">
        ...
        <div className="max-w-[1280px] mx-auto">
          ...
          <div className="flex flex-col lg:flex-row gap-6 lg:gap-8">
            {/* Sidebar - Fixed on desktop, hidden on mobile with fade animation */}
            <aside
              id="filtersSidebar"
              className={`hidden lg:block w-80 fixed top-16 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 overflow-y-auto shadow-sm z-10 transition-all duration-300 ease-in-out ${
                showDesktopSidebar
              ...
              style={{
                ...
                bottom:
                  sidebarBottomOffset > 0 ? `${sidebarBottomOffset}px` : "auto",
              }}
            >
            ...
            {/* Collapsed rail (desktop) with fade animation */}
            {!showDesktopSidebar && (
              <div
                className="hidden lg:flex flex-col items-center gap-2 w-14 fixed top-16 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 z-10 py-4 transition-all duration-300 opacity-100"
                style={{
                  height: `calc(100vh - 4rem - ${sidebarBottomOffset}px)`,
                  maxHeight: `calc(100vh - 4rem - ${sidebarBottomOffset}px)`,
                  bottom:
                    sidebarBottomOffset > 0
                      ? `${sidebarBottomOffset}px`
                      : "auto",
                }}
              >
                <Button
            ...
            <section
              className={`flex-1 transition-all duration-300 ease-in-out ${
                showDesktopSidebar ? "lg:ml-[336px]" : "lg:ml-[72px]"
              }`}
            >
              ...
              <div className="hidden h-fit md:flex md:items-center bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-5 mb-6">
                <div className="flex flex-1 flex-col md:flex-row gap-4">
                  ...
                  <div className="flex gap-2 flex-col">
                    <Select value={sortBy} onValueChange={handleSortChange}>
                      <SelectTrigger className="w-full flex-1 h-24 md:w-60 rounded-lg border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="domainRank-desc">
                          Domain Rank: High to Low
                        </SelectItem>
                        <SelectItem value="domainAuthority-desc">
                          Domain Authority: High to Low
                        </SelectItem>
              ...
              <div className="mb-6 flex justify-between items-center">
                <div className="text-gray-600 dark:text-gray-300">
                  <span className="font-medium">{filteredDomains.length}</span>{" "}
                  of <span className="font-medium">{domains.length}</span>{" "}
                  domains
              ...
              {/* Domain Cards - Max 2 columns */}
              {loading ? (
                <div
                  className={
                    viewMode === "grid"
                      ? `grid md:grid-cols-2 gap-6 ${
                          showDesktopSidebar ? "" : "lg:grid-cols-3"
                        }`
                      : "space-y-4"
                  }
                >
          ...
          <div className="grid grid-cols-1 gap-2 max-h-[70vh] overflow-y-auto py-2">
            {(
              [
                ...
                {
                  key: "domainAuthority-desc",
                  label: "Domain Authority: High to Low",
                },
                { key: "score-desc", label: "Score: High to Low" },
                { key: "age-desc", label: "Age: Oldest First" },
                {
                  key: "referringDomains-desc",
                  label: "Referring Domains: High to Low",
                },
                {
                  key: "monthlyTraffic-desc",
                  label: "Monthly Traffic: High to Low",
                },
