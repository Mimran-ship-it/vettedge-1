...
export function Header() {
  ...
  return (
    <>
      ...
      <header
        className={`bg-white dark:bg-gray-900 shadow-sm border-b border-gray-200 dark:border-gray-700 w-full z-50 fixed top-0 left-0 transition-transform duration-300 ease-in-out ${
      ...
      >
        <div className="max-w-8xl mx-auto px-2 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <nav className="hidden lg:flex items-center space-x-4 xl:space-x-10">
              <Link href="/domains" className={linkClass("/domains")}>
                Buy Domains
              </Link>
              <Link
                href="/vetting-process"
                className={linkClass("/vetting-process")}
              >
                Vetting Process
              </Link>
              ...
              <div className="flex items-center space-x-2">
                {user ? (
                  ...
                  <div className="flex items-center justify-center">
                    {/* User Auth Dropdown (when not logged in) */}
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button
                          variant="ghost"
              ...
              <div className="p-5">
                ...
                {user && (
                  <div className="pt-4 border-t mt-4">
                    <div className="flex flex-col space-y-3">
                      <Link
                        href="/dashboard"
                        className={`flex items-center px-3 py-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 ${linkClass(
                          "/dashboard"
                        )}`}
                        onClick={() => setIsMenuOpen(false)}
                      >
                ...
                {!user && (
                  <div className="pt-4 flex flex-col space-y-3 border-t mt-4">
                    <Link
                      href="/auth/signin"
                      className={linkClass("/auth/signin")}
                      onClick={() => setIsMenuOpen(false)}
                    >
                      Sign In
                    </Link>
